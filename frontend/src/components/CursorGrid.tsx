...
// Grade reativa ao cursor (React Bits — CursorGrid), adaptada para o projeto:
// TypeScript, classes Tailwind no lugar do CSS próprio e eventos de pointer na
// window — o componente vive numa camada `pointer-events-none` atrás do
// conteúdo (ex.: fundo do login), então ouvir no container mataria o efeito
// sempre que o cursor estivesse sobre o card ou o painel de marca.

type FalloffCurve = 'linear' | 'smooth' | 'sharp';

const FALLOFF_CURVES: Record<FalloffCurve, (t: number) => number> = {
  linear: (t) => t,
  smooth: (t) => t * t * (3 - 2 * t),
  sharp: (t) => t * t * t,
};
...
const CursorGrid = ({
  ...
  useEffect(() => {
    ...
    // Estado da grade: um par alpha + timestamp por célula, row-major.
    let cols = 0;
    let rows = 0;
    let offX = 0;
    let offY = 0;
    ...
    // Acende cada célula cujo centro cai dentro do raio, com a curva de
    // falloff configurada mapeando distância para brilho.
    ...
    const draw = (now: number) => {
      ...
      // Pulsos de clique entregam energia às células conforme se expandem
      ...
      for (let i = 0; i < alphas.length; i++) {
        ...
        ctx.beginPath();
        if (p.cellRadius > 0 && 'roundRect' in ctx) {
  ...
  // Repinta as camadas estáticas quando props visuais mudam em repouso
  useEffect(() => {
    wakeRef.current?.();
