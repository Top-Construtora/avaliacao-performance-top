...
const EvaluationSection: React.FC<EvaluationSectionProps> = ({
  ...
  const sectionProgress =
    (section.items.filter((item) => item.score !== undefined).length / section.items.length) * 100;
    ...
    setSections((prev) =>
      prev.map((s) =>
        s.id === sectionId
          ? {
              ...s,
              items: s.items.map((item) => (item.id === itemId ? { ...item, score } : item)),
            }
          : s,
      ),
    );
  ...
  const ratingLabels: Record<number, { label: string; color: string; darkColor: string }> = {
    1: { label: 'Insatisfatório', color: 'bg-destructive', darkColor: '' },
    2: { label: 'Em Desenvolvimento', color: 'bg-warning', darkColor: '' },
    3: { label: 'Satisfatório', color: 'bg-success/80', darkColor: '' },
    4: { label: 'Excepcional', color: 'bg-success', darkColor: '' },
  };
      ...
      >
        ...
        >
          <div className="flex items-center space-x-3 sm:space-x-4 min-w-0 flex-1">
            <div
              className={`p-2 sm:p-3 rounded-lg sm:rounded-xl bg-gradient-to-br ${section.gradient} ${section.darkGradient} shadow-md dark:shadow-lg flex-shrink-0`}
            >
              <IconComponent className="h-5 w-5 sm:h-6 sm:w-6 text-white" />
            </div>
            <div className="text-left min-w-0 flex-1">
              <h2 className="text-lg sm:text-xl font-bold text-foreground flex flex-col sm:flex-row sm:items-center font-lemon-milk tracking-wide">
                <span className="truncate">{section.title}</span>
                <span
                  className={`mt-1 sm:mt-0 sm:ml-3 text-xs font-medium px-2 py-1 rounded-full ${section.bgColor} ${section.darkBgColor} text-foreground flex-shrink-0`}
                >
                  Peso {section.weight}%
                </span>
              </h2>
              <p className="text-xs sm:text-sm text-muted-foreground mt-1">
                {section.items.filter((item) => item.score !== undefined).length} de{' '}
                {section.items.length} competências avaliadas
              </p>
            </div>
          </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between space-y-2 sm:space-y-0">
                    ...
                    {item.score && getSafeRating(item.score) && (
                      <div className="text-center sm:text-right flex-shrink-0">
                        <div
                          className={`px-3 py-1 rounded-full text-xs sm:text-sm font-medium ${getSafeRating(item.score)!.color} ${getSafeRating(item.score)!.darkColor} text-white`}
                        >
                    ...
                    {[1, 2, 3, 4].map((rating) => {
                      ...
                      return (
                        <button
                          key={rating}
                          onClick={() =>
                            !readOnly && handleScoreChange(section.id, item.id, rating)
                          }
                          disabled={readOnly}
        ...
        >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="bg-card p-4 sm:p-6 rounded-xl border border-border">
              <h4 className="text-sm font-medium text-muted-foreground mb-1">Técnicas</h4>
              <p className="text-2xl sm:text-3xl font-bold text-foreground">
                {scores.technical.toFixed(3)}
              </p>
            ...
            <div className="bg-card p-4 sm:p-6 rounded-xl border border-border">
              <h4 className="text-sm font-medium text-muted-foreground mb-1">Comportamentais</h4>
              <p className="text-2xl sm:text-3xl font-bold text-foreground">
                {scores.behavioral.toFixed(3)}
              </p>
            ...
            <div className="bg-card p-4 sm:p-6 rounded-xl border border-border">
              <h4 className="text-sm font-medium text-muted-foreground mb-1">Organizacionais</h4>
              <p className="text-2xl sm:text-3xl font-bold text-foreground">
                {scores.organizational.toFixed(3)}
              </p>
            ...
            <div className="bg-lime text-obsidian p-4 sm:p-6 rounded-xl">
              <h4 className="text-sm font-medium text-obsidian/70 mb-1">Nota Final</h4>
              <p className="text-2xl sm:text-3xl font-bold">{scores.final.toFixed(3)}</p>
              <p className="text-xs text-obsidian/70 mt-1">Média Ponderada</p>
