...
const PDIViewer: React.FC<PDIViewerProps> = ({ pdiData }) => {
  ...
  const renderActionItems = (items: ActionItem[], title: string, bgColor: string) => {
    ...
    return (
      <div className="mb-8">
        <h3 className={`text-lg font-semibold mb-4 ${bgColor} p-3 rounded-lg`}>{title}</h3>
          ...
          {items.map((item) => (
            <div key={item.id} className="bg-white p-4 rounded-lg border border-gray-200 shadow-sm">
              <div className="flex items-start justify-between mb-3">
                ...
                <div
                  className={`flex items-center gap-1 px-3 py-1 rounded-md text-sm font-medium ${getStatusColor(item.status)}`}
                >
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      {(pdiData.dataCriacao || pdiData.dataAtualizacao) && (
        <div className="mt-6 text-sm text-gray-500 text-center">
          {pdiData.dataCriacao && <p>Criado em: {formatDate(pdiData.dataCriacao)}</p>}
