...
export const useSalaryManagement = () => {
  ...
  const calculateSalaryWithValidation = useCallback(
    async (
  ...
  const processProgression = useCallback(
    async (
      userId: string,
    ...
    try {
      ...
      for (const row of data) {
        try {
          ...
          // Atribuir ao usuário
          await salaryService.assignUserToTrack(user.id, trackPosition.id, level.id);
  ...
  const analyzeSalaryEquity = useCallback(async (departmentId?: string, positionId?: string) => {
    try {
  ...
  const simulateBudgetImpact = useCallback(
    async (
      changes: Array<{
        userId: string;
        newSalary: number;
      ...
      const employerCosts = {
        inss: totalIncrease * 0.2, // 20% INSS patronal
