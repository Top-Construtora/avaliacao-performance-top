...
const UserRegistration = () => {
  ...
  useEffect(() => {
    const loadPositions = async () => {
      ...
      try {
        // Primeiro, buscar todos os track_positions
        ...
        // Agora, buscar os nomes dos cargos da tabela job_positions
        if (trackPositions && trackPositions.length > 0) {
          ...
          if (!positionsError && positionsData) {
            // Criar um mapa de position_id -> position data
            const positionsMap = positionsData.reduce((acc, pos) => {
              acc[pos.id] = pos;
              return acc;
            }, {} as Record<string, any>);

            // Adicionar os dados de job_positions aos track_positions
            ...
            // Se falhar, usar apenas os track_positions
  ...
  useEffect(() => {
    if (formData.departmentId) {
      ...
      // Limpar trilha e cargo selecionados se não pertencem ao novo departamento
  ...
  useEffect(() => {
    if (formData.trackId) {
      ...
      // Limpar cargo selecionado se não pertence à nova trilha
  ...
  // Mapear order_index para internível (A-E)
  ...
  // Atualizar internível quando o cargo é selecionado
  useEffect(() => {
    if (formData.positionId) {
      const selectedPosition = positions.find(p => p.id === formData.positionId);
      if (selectedPosition) {
        // Usar o nome da tabela positions, senão usar position_id
