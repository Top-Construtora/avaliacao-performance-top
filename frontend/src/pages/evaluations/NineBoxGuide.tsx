...
const NineBoxGuide = () => {
  ...
  const boxes = [
    // Linha Superior (Alto Potencial)
    {
      ...
      details: {
        descricao:
          'Excede expectativas quanto ao potencial. Pode estar na função errada ou ser iniciante.',
        ...
        desenvolvimento:
          'Avaliar habilidades funcionais/técnicas. Plano detalhado para construir novas habilidades',
        recompensa:
          'Baixo ou nenhum aumento no salário base. Incentivo simbólico se houver evolução',
    ...
    },
    {
      ...
      details: {
        descricao:
          'Atende e muitas vezes excede expectativas. Potencial para mudanças na carreira.',
        atribuicoes:
          'Trabalhos desafiadores e variados a cada 12-24 meses com aumento de responsabilidades',
        ...
        recompensa:
          'Crescimento agressivo no salário-base. Pagar acima do mercado, bonificações de retenção',
        engajamento:
          'Liderar forças-tarefas interfuncionais. Convites especiais para eventos externos',
      },
    },
    {
      id: '9',
      ...
      icon: Rocket,
      bgColor:
        'bg-gradient-to-br from-gray-50 to-teal-50 dark:from-gray-900/20 dark:to-teal-900/20',
      ...
      details: {
        ...
        atribuicoes:
          'Trabalhos desafiadores a cada 12-24 meses. Atribuições com alto risco e pensamento estratégico',
        mentoria: 'Mentor de nível superior dos boxes 6, 8 ou 9. Mentores de outras divisões',
        desenvolvimento:
          'Coaching de alto grau com avaliação contínua e desenvolvimento de competências críticas',
        recompensa: 'Agressivo na base e incentivos. Pagar acima do mercado. Bonificação de ações',
        engajamento:
          'Liderar forças-tarefas globais-chave. Reuniões externas. Monitoramento sério do RH',
      },
    },
    // Linha do Meio (Médio Potencial)
    {
      ...
      details: {
        descricao:
          'Pode ser novo ou não apresentar competência para função. Tem potencial não demonstrado.',
    ...
    },
    {
      ...
      details: {
        descricao:
          'Atende consistentemente e ocasionalmente excede expectativas. Promovível um nível.',
        atribuicoes: 'Projetos com metas importantes e prazo apertado. Gerenciar grupo temporário',
        mentoria:
          'Mentor do Box 8 (desenvolvimento vertical) ou Box 6 (desenvolvimento horizontal)',
    ...
    // Linha Inferior (Baixo Potencial)
    {
      ...
      details: {
        ...
        atribuicoes:
          'Determinar o problema. Passar para atribuição apropriada ou encaminhar demissão',
    ...
    },
    {
      ...
      details: {
        ...
        engajamento:
          'Atividades onde conhecimentos sejam valorizados. Exposição a alto performance',
      },
    },
    {
      id: '7',
  ...
  const MatrixView = () => (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
      {/* Matrix */}
      <div className="bg-card rounded-xl sm:rounded-2xl shadow-sm dark:shadow-lg border border-border p-4 sm:p-6">
        <div className="relative">
          {/* Potencial Label - Vertical Left */}
          <div className="absolute -left-0 top-1/2 -translate-y-1/2">
            <span className="text-sm font-medium text-muted-foreground -rotate-90 block whitespace-nowrap">
              Potencial →
            </span>
          </div>
          ...
          <div className="max-w-md mx-auto px-8">
            ...
            <div className="grid grid-cols-3 gap-2 mb-2">
              <div className="text-center">
                <span className="text-xs sm:text-sm font-medium text-muted-foreground">Baixo</span>
              </div>
              <div className="text-center">
                <span className="text-xs sm:text-sm font-medium text-muted-foreground">Médio</span>
              </div>
              <div className="text-center">
                <span className="text-xs sm:text-sm font-medium text-muted-foreground">Alto</span>
              </div>
            </div>
                  ...
                  return (
                    <div
                      ...
                      className={`
                        ...
                        ${selectedBox === box.id ? 'ring-2 ring-offset-1 dark:ring-offset-background ring-[#D2FF00] shadow-lg dark:shadow-xl' : ''}
                        rounded-lg
                      `}
                    >
                      <div className="flex flex-col items-center justify-center h-full space-y-1">
                        ...
                        <span
                          className={`text-xs font-medium ${box.textColor} text-center leading-tight`}
                        >
                          {box.title}
                        </span>
                      </div>
      ...
      {/* Side Information */}
      <div className="bg-card rounded-xl sm:rounded-2xl shadow-sm dark:shadow-lg border border-border p-4 sm:p-6 h-fit">
        {selectedBox ? (
          <>
            {(() => {
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className={`${box.bgColor} ${box.borderColor} border rounded-lg p-3`}>
                      ...
                      <p className={`font-semibold capitalize ${box.textColor}`}>
                        {box.performance}
                      </p>
                    </div>
                    <div className={`${box.bgColor} ${box.borderColor} border rounded-lg p-3`}>
                  ...
                  <div className="bg-secondary rounded-lg p-4">
                    ...
                    <p className="text-sm text-muted-foreground">{box.details.descricao}</p>
                  </div>

                  {/* Action Items */}
                  <div className="space-y-3">
                    <div className="flex items-start gap-3">
                      <Briefcase className="w-4 h-4 text-lime-deep dark:text-lime mt-0.5 flex-shrink-0" />
                      <div className="flex-1">
                        <h5 className="text-sm font-medium text-foreground">Atribuições</h5>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {box.details.atribuicoes}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-start gap-3">
                      <UserCheck className="w-4 h-4 text-lime-deep dark:text-lime mt-0.5 flex-shrink-0" />
                      <div className="flex-1">
                        <h5 className="text-sm font-medium text-foreground">Mentoria</h5>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {box.details.mentoria}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-start gap-3">
                      <GraduationCap className="w-4 h-4 text-lime-deep dark:text-lime mt-0.5 flex-shrink-0" />
                      <div className="flex-1">
                        <h5 className="text-sm font-medium text-foreground">Desenvolvimento</h5>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {box.details.desenvolvimento}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-start gap-3">
                      <DollarSign className="w-4 h-4 text-lime-deep dark:text-lime mt-0.5 flex-shrink-0" />
                      <div className="flex-1">
                        <h5 className="text-sm font-medium text-foreground">Recompensa</h5>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {box.details.recompensa}
                        </p>
                      </div>
                    </div>

                    <div className="flex items-start gap-3">
                      <Target className="w-4 h-4 text-lime-deep dark:text-lime mt-0.5 flex-shrink-0" />
                      <div className="flex-1">
                        <h5 className="text-sm font-medium text-foreground">Engajamento</h5>
                        <p className="text-xs text-muted-foreground mt-0.5">
                          {box.details.engajamento}
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-muted-foreground">
            ...
            <p className="text-center text-sm mt-1">
              Clique em qualquer posição para ver os detalhes
            </p>
          </div>
        )}
      </div>
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
          ...
          <div className="flex items-center space-x-3 sm:space-x-4 w-full lg:w-auto">
            <div className="flex-1 lg:flex-none">
              <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold text-foreground flex items-center">
                <BookOpen className="h-6 w-6 sm:h-7 sm:w-7 lg:h-8 lg:w-8 text-lime-deep dark:text-lime mr-2 sm:mr-3" />
      ...
      <motion.div
        ...
        className="bg-card rounded-xl sm:rounded-2xl shadow-sm dark:shadow-lg border border-border p-4 sm:p-6"
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="space-y-3">
            <h4 className="font-medium text-foreground text-sm">Médio Potencial</h4>
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs">
                <div className="w-3 h-3 bg-red-400 dark:bg-red-600 rounded"></div>
                <span className="text-muted-foreground">Baixo performance: Avaliar adequação</span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <div className="w-3 h-3 bg-yellow-400 dark:bg-yellow-600 rounded"></div>
                <span className="text-muted-foreground">
                  Médio performance: Investir para manter
                </span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <div className="w-3 h-3 bg-teal-400 dark:bg-teal-600 rounded"></div>
                <span className="text-muted-foreground">Alto performance: Considerar promoção</span>
              </div>
            </div>
          </div>

          <div className="space-y-3">
            <h4 className="font-medium text-foreground text-sm">Baixo Potencial</h4>
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs">
                <div className="w-3 h-3 bg-red-400 dark:bg-red-600 rounded"></div>
                <span className="text-muted-foreground">
                  Baixo performance: Avaliar movimentação
                </span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <div className="w-3 h-3 bg-yellow-400 dark:bg-yellow-600 rounded"></div>
                <span className="text-muted-foreground">
                  Médio performance: Movimentação horizontal
                </span>
              </div>
              <div className="flex items-center gap-2 text-xs">
                <div className="w-3 h-3 bg-teal-400 dark:bg-teal-600 rounded"></div>
                <span className="text-muted-foreground">
                  Alto performance: Manter e revisar remuneração
                </span>
              </div>
            </div>
          </div>
