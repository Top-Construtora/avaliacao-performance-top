...
const UserManagement = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    teamName: '',
    teamDepartmentId: '',
    teamResponsibleId: '', // Changed from teamLeaderId
    ...
    departmentGoals: '',
    departmentResponsibleId: '', // New field
  });
    ...
    if (item) {
      if (type === 'user') {
        ...
        setFormData({
          ...
          teamResponsibleId: item.leaderId, // Using leaderId as responsibleId
          teamMemberIds: item.memberIds,
          teamDescription: item.description || '',
        });
      } else if (type === 'department') {
        // Find responsible for department (can be any user)
  ...
  const handleSubmit = () => {
    ...
    if (modalType === 'user') {
      const userData = {
        ...
        leaderOfTeamIds: [], // No longer using this for team leadership
      ...
      const teamData = {
        ...
        leaderId: formData.teamResponsibleId, // Using leaderId to store responsibleId
                        ...
                        <div key={dept.id} className="border-b border-gray-100 last:border-0">
                          ...
                          <div className="p-2">
                            {deptTeams.map(team => {
                              ...
                              const responsible = getUserById(team.leaderId); // Using leaderId as responsibleId
